...
export default async function PropertyDetailsPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return (
    <div className="pt-32 pb-20 px-6 lg:px-12 xl:px-16 max-w-[1600px] mx-auto w-full min-h-screen">
      <Link href="/properties" className="inline-flex items-center gap-2 text-sm font-medium text-black/60 hover:text-dark transition-colors mb-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-10">
          ...
          <div>
            <div className="flex justify-between items-start mb-4">
              <div>
                <h1 className="text-3xl font-medium tracking-tight mb-2">{property.title}</h1>
                <p className="text-lg text-black/60">{property.city} • {property.area}m² • {property.rooms} Rooms</p>
              </div>
              <div className="bg-brand/10 text-brand font-semibold px-4 py-2 rounded-full text-sm">
                {property.code}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-6 gap-x-4">
              <div>
                <p className="text-sm text-black/50 mb-1">Floor</p>
                <p className="font-medium">{property.floor}</p>
              </div>
              <div>
                <p className="text-sm text-black/50 mb-1">Balcony</p>
                <p className="font-medium">{property.balcony}</p>
              </div>
              <div>
                <p className="text-sm text-black/50 mb-1">Parking</p>
                <p className="font-medium">{property.parking}</p>
              </div>
              <div>
                <p className="text-sm text-black/50 mb-1">Furnished</p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {property.amenities.map(amenity => (
                <div key={amenity} className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm text-sm font-medium text-black/70 border border-black/5">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {property.nearby.map(place => (
                <div key={place.name} className="flex justify-between items-center bg-card p-4 rounded-xl shadow-sm">
                  <span className="font-medium text-sm text-dark">{place.name}</span>
                  <span className="text-sm text-black/50 bg-white px-3 py-1 rounded-full">{place.distance}</span>
                </div>
              ))}
            </div>
          </div>
